...
{
  std::vector<std::string> dirs{"data/src/faster-rcnn/", "data/faster-rcnn/"};
...
void run_cam(void)
{
  ...
  while (true) {
    ...
    cv::resize(frame, frame, cv::Size(500, 375));
    img_scale_ratio_w=float(frame.size().width)/float(frame.size().width);	//beaware the int/int rounding 
    img_scale_ratio_h=float(frame.size().height)/float(frame.size().height);
    //std::vector<int> params;
    //params.push_back(cv::IMWRITE_PXM_BINARY);
    //params.push_back(1);
    //cv::imwrite("../data/faster-rcnn/realTime.ppm", frame, params);
    // read a random sample image
    //srand(unsigned(time(nullptr)));
    // available images
    //std::vector<std::string> imageList = { /*"0001.ppm",*/ "000456.ppm",  "000542.ppm",  "001150.ppm", "001763.ppm", "004545.ppm" };
    //std::vector<std::string> imageList = { "realTime.ppm" };
    //std::vector<std::string> imageList = { "realTime.ppm" };
